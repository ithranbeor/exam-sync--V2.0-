  ...
  const SchedulerView: React.FC<SchedulerViewProps> = ({ user }) => {
    ...
    const getSectionDisplay = (exam: ExamDetail): string => {
      if (exam.sections && exam.sections.length > 0) {
        ...
        // ✅ Enhanced range detection for sections like IT4R1_Track1 - IT4R9_Track1
        // Group sections by prefix AND suffix (everything before and after the number)
        const groups: { [key: string]: { num: number, original: string }[] } = {};
        
        exam.sections.forEach(section => {
          // Match: prefix + number + optional suffix
          const match = section.match(/^(.*?)(\d+)(.*)$/);
          if (match) {
            const prefix = match[1];
            const num = parseInt(match[2]);
            const suffix = match[3];
            const groupKey = `${prefix}|${suffix}`; // Group by prefix+suffix combo
            
            if (!groups[groupKey]) groups[groupKey] = [];
            groups[groupKey].push({ num, original: section });
          } else {
        ...
        // Build ranges for each group (sort by first number in group for consistent ordering)
        const result: string[] = [];
        
        // Sort group keys by the first number in each group
        const sortedGroupKeys = Object.keys(groups).sort((a, b) => {
          const groupA = groups[a];
          const groupB = groups[b];
          if (groupA[0].num === 0 && groupB[0].num === 0) {
            return a.localeCompare(b);
          }
          const minA = Math.min(...groupA.map(item => item.num));
          const minB = Math.min(...groupB.map(item => item.num));
          return minA - minB;
        });
        
        sortedGroupKeys.forEach(groupKey => {
          const items = groups[groupKey].sort((a, b) => a.num - b.num);
