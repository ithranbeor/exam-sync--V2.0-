...
import {
  ...
  FaSignOutAlt, FaPenAlt, FaBars, FaTimes, FaUserShield, FaBell, FaUser, FaClipboardList, FaClock, FaCalendarAlt, FaCheckCircle, FaChevronDown, FaChevronLeft, FaChevronRight, FaListUl
} from 'react-icons/fa';
...
const DashboardAdmin: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const alreadyShown = sessionStorage.getItem('roleModalShown');
    if (!alreadyShown && roles.some(r => r !== 'admin')) {
      setRoleModal(true);
      sessionStorage.setItem('roleModalShown', '1');
    }
  }, [roles]);

  const handleRoleModalContinue = (dest: 'admin' | 'faculty') => {
    setRoleModal(false);
    if (dest === 'faculty') navigate('/faculty-dashboard');
  };
      ...
      default: return (
        <div className="dh-root">
          ...
          <div className="dh-shortcuts-bar">
            {[
              { key: 'structure',           label: 'College Structure',  icon: <FaBuilding size={18} />,          desc: 'Manage college structure' },
              { key: 'courses',             label: 'Courses & Sections', icon: <FaClipboardList size={18} />,     desc: 'Manage courses & sections' },
              { key: 'buildings-and-rooms', label: 'Buildings & Rooms',  icon: <PiBuildingsFill size={18} />,     desc: 'Manage buildings & rooms' },
              { key: 'exam-period',         label: 'Exam Period',        icon: <IoCalendarSharp size={18} />,     desc: 'Set exam periods' },
              { key: 'set-Modality',        label: 'Set Modality',       icon: <FaPenAlt size={18} />,            desc: 'Configure modality' },
              { key: 'User Management',     label: 'User Management',    icon: <FaUsers size={18} />,             desc: 'Manage users & roles' },
            ].map(sc => (
              <button
                key={sc.key}
                type="button"
                className="dh-shortcut-btn"
                onClick={() => handleMenuClick(sc.key)}
              >
              ...
              <div className="dh-section-card">
                ...
                <div className="dh-cal-header">
                  ...
                  <div className="dh-cal-controls">
                    ...
                    {/* Filter dropdown */}
                    <div className="dh-filter-wrapper" ref={calendarDropdownRef}>
                      <button
                        type="button"
    ...
    <div className="app-container">
                  ...
                  {dropdownOpen && (
                    <div className="topbar-dropdown">
                      ...
                      <ul className="dropdown-menu-list">
                        ...
                        {hasFacultyRole && (
                          <li><button type="button" onClick={() => navigate('/faculty-dashboard')}><FaUserShield size={13} /> Switch to Faculty</button></li>
                        )}
      ...
      {roleModal && (
        <div className="myModal-overlay">
          <div className="myModal-box" style={{ maxWidth: 420 }} onClick={e => e.stopPropagation()}>
            <div style={{ textAlign: 'center', marginBottom: 8 }}>
              <FaUserShield size={32} style={{ color: 'var(--ul-navy, #0d2340)', marginBottom: 8 }} />
            </div>
            <h3 className="myModal-title">You have multiple roles</h3>
            <p style={{ fontSize: 13, color: '#6b84a0', textAlign: 'center', marginBottom: 16, lineHeight: 1.6 }}>
              Your account has both <strong>Admin</strong> and <strong>Faculty</strong> access.<br />
              You can switch dashboards anytime by clicking your <strong>profile avatar</strong> in the top-right corner.
            </p>

            <div style={{ display: 'flex', gap: 8, flexDirection: 'column', marginBottom: 16 }}>
              <div style={{ background: '#f8f9fb', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#3a5070', display: 'flex', alignItems: 'center', gap: 10 }}>
                <FaUserShield size={14} style={{ flexShrink: 0 }} />
                <span><strong>Admin Dashboard</strong> — Manage structure, courses, rooms, exam periods, users.</span>
              </div>
              <div style={{ background: '#f8f9fb', borderRadius: 10, padding: '10px 14px', fontSize: 13, color: '#3a5070', display: 'flex', alignItems: 'center', gap: 10 }}>
                <FaUser size={14} style={{ flexShrink: 0 }} />
                <span><strong>Faculty Dashboard</strong> — Set availability, view schedules, proctor exams.</span>
              </div>
            </div>

            <p style={{ fontSize: 12, color: '#9ab', textAlign: 'center', marginBottom: 20 }}>Where would you like to continue?</p>

            <div className="myModal-actions">
              <button
                type="button"
                className="myModal-btn myModal-btn-confirm"
                onClick={() => handleRoleModalContinue('admin')}
              >
                Continue as Admin
              </button>
              <button
                ...
                onClick={() => handleRoleModalContinue('faculty')}
              >
                Switch to Faculty
              </button>
            </div>
